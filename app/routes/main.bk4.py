...
def process_file(app, task_id, file_path, selected_columns, row_limit, api_key, instructions, gpt_model, user_id, additional_columns):
    ...
    with app.app_context():
        try:
            ...
            if row_limit and str(row_limit).strip():
                try:
                    ...
                    if row_limit > 0:
                        ...
                        logger.info(f"Applied row limit: processing first {row_limit} rows")
                except ValueError:
            ...
            with ThreadPoolExecutor(max_workers=5) as executor:  # Hardcoded to 5 workers
                futures = []
                for _, row in df.iterrows():
                    future = executor.submit(
                        ...
                        additional_columns=additional_columns,  # Pass additional columns here
                        gpt_model=gpt_model,
                        scrapeops_client=None  # Remove scrapeops dependency for now
                    )
            ...
            try:
                ...
                socketio.emit('complete', {
                    ...
                    'success_rate': f"{(len(results) / total_rows) * 100:.2f}%" if total_rows > 0 else "0.00%"
                }, namespace='/')
                return True
            ...
            socketio.emit('error', {
                'status': 'error',
                'message': f"Process file error: {str(e)}"
            }, namespace='/')
            return False
    ...
    try:
        ...
        thread = threading.Thread(
            ...
            args=(
                ...
                current_user.id,  # Pass the current user's ID
                additional_columns
            )
        )
        thread.daemon = True
